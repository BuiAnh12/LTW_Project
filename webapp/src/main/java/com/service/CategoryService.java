...
public class CategoryService {
    ...
    public Page<ResGroupDto> getGroupListPage(HttpServletRequest request, Model model,
        ...
        for (Group group : groupList) {
        	...
        	if (!isNullOrEmpty(Sstudent) && registrationRepository.countByGroupId(group.getId())!=Long.valueOf(Sstudent)) {
                continue;
            }
        	if (!isNullOrEmpty(Steacher) && !group.getMainTeacher().getName().toLowerCase().contains(Steacher.toLowerCase())) {
                continue;
            }
        	if (!isNullOrEmpty(Ssupervisor) && !group.getSupervisor().getName().toLowerCase().contains(Ssupervisor.toLowerCase())) {
                continue;
            }
        	if (!isNullOrEmpty(Scourse) && !group.getCourse().getCourseName().toLowerCase().contains(Scourse.toLowerCase())) {
                continue;
            }
